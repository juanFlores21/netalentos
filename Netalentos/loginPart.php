...
if (empty($usuario) || empty($contra)) {
    ...
    $sql = "SELECT * FROM Participante WHERE (nombre = '$usuario' OR Email = '$usuario' OR idParticipante = '$usuario') AND pwd = '$contra'"; // Cambiar 'nombre' por 'usuario'
    ...
    if ($resultado) {
        if (mysqli_num_rows($resultado) == 1) {
            ...
            $_SESSION['idParticipante'] = $fila['idParticipante'];
            $_SESSION['nombre'] = $fila['nombre'];
            $_SESSION['Email'] = $fila['Email'];
            header("Location: menuPart.php");
