...
?>
        ...
        // Consultar si el correo ya está registrado en la tabla Juez
        $sqlCorreoJuez = "SELECT COUNT(*) as total FROM Juez WHERE Email = '$correo'";
        $resultCorreoJuez = $conn->query($sqlCorreoJuez);
        $rowCorreoJuez = $resultCorreoJuez->fetch_assoc();
        $correoExistenteJuez = $rowCorreoJuez['total'];

        if ($correoExistenteJuez > 0) {
        ...
        ?>
            ...
            if ($resultNacionalidad->num_rows > 0 && $resultGenero->num_rows > 0) {
                ...
                // Insertar los datos en la tabla Juez
                $sql = "INSERT INTO Juez (idGenero, idNacionalidad, idCiudad, Nombre, Paterno, Materno, Edad, FechaNac, Email, Calle, Num, Colonia, CodigoPostal, pwd) 
